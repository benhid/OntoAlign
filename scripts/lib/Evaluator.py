...
def threshold_searching(Y, scores, num):
    max_valid_f1, max_valid_p, max_valid_r, max_valid_acc, max_alpha = (
        -np.inf,
        -np.inf,
        -np.inf,
        -np.inf,
    ...
    for alpha in np.arange(0, 1, 0.02):
        valid_p, valid_r, valid_f1, valid_acc = cal_result_score(
            Y=Y, scores=scores, alpha=alpha, n=num
        )
        print(
            "alpha: %.2f, precision: %.3f, recall: %.3f, f1 score: %.3f, accuracy: %.3f"
            % (alpha, valid_p, valid_r, valid_f1, valid_acc)
        )
        if valid_f1 > max_valid_f1:
            max_alpha, max_valid_f1, max_valid_p, max_valid_r, max_valid_acc = (
                alpha,
                valid_f1,
                valid_p,
                valid_r,
