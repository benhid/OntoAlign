...
parser.add_argument(
    ...
    type=float,
    default=0.65,
    help="this threshold often needs to be fine-tuned " "to get the best F1 score",
)
parser.add_argument(
    "--prediction_sample_num",
    type=int,
    default=50,
    help="how many mappings to sample for human annotation "
    "for precision approximation",
)
parser.add_argument(
    "--sample_to_annotate_file", type=str, default="LogMap-ML_samples_a.txt"
)
parser.add_argument(
    "--output_mapping_file", type=str, default="LogMap-ML_output_mappings.txt"
)
...
def cal_recall(mappings_dict):
    ...
    for gs in GS:
        ...
        if (
            gs_left_class in mappings_dict
            and gs_right_class in mappings_dict[gs_left_class]
        ):
            recall_num += 1
...
def read_logmap_mapping(filename):
    ...
    with open(filename) as f:
        for line in f.readlines():
            m = line.strip().split("|")
            left_c = replace_with_prefix(m[0])
            right_c = replace_with_prefix(m[1])
...
with open(FLAGS.prediction_out_file) as f:
    ...
    for j in range(0, len(lines), 3):
        ...
        if float(tmp[3]) >= FLAGS.threshold:
            if not sample.violate_branch_conflict(
                p1_str=mapping_n[0], p2_str=mapping_n[1]
            ):
...
for left_c in logmap_anchors:
    for right_c in logmap_anchors[left_c]:
        ...
        if not sample.violate_branch_conflict(
            p1_str=label_split[0], p2_str=label_split[1]
        ):
...
sample_save_mapping(
    filename=FLAGS.sample_to_annotate_file, mappings_dict=hybrid_left_right
)
