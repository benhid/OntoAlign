...
for sample in LML_samples:
    ...
    if "%s|%s" % (tmp[0], tmp[1]) not in GS:
        ...
        if len(tmp) >= 4 and tmp[3] == "true":
            sv_n += 1
print(
    "LogMap-ML: sampled mappings not in GS: %d, correct samples: %d, sampled precision: %f"
    % (s_n, sv_n, sv_n / s_n)
)
...
samples = set()
filenames = [
    LogMap_ML_annotated_sample_file,
    LogMap_annotated_sample_file,
    AML_annotated_sample_file,
]
...
for line in open(LogMap_output_mapping_file).readlines():
    ...
    c1 = replace_with_prefix(uri=tmp[0])
    c2 = replace_with_prefix(uri=tmp[1])
