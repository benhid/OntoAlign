...
parser.add_argument(
    "--anchor_mapping_file",
    type=str,
    default="FMA-NCI-small/logmap_output/logmap_anchors.txt",
)
                    ...
                    for i, v in enumerate(c.getchildren()):
                        if i < 2:
                            for value in v.attrib.values():
                                mapping.append(replace_with_prefix(value).lower())
                                break
                        if i == 3:
                            mv = v.text
...
if __name__ == "__main__":
    ...
    ref_mappings_str, ref_all_mappings_str = read_oaei_mappings(
        file_name=FLAGS.oaei_GS_file
    )
    ...
    with open(FLAGS.anchor_mapping_file) as f:
        for line in f.readlines():
            ...
            anchor_mappings_str.append(
                "%s|%s"
                % (
                    replace_with_prefix(tmp[0]).lower(),
                    replace_with_prefix(tmp[1]).lower(),
                )
            )
    ...
    F1 = 2 * P * R / (P + R)
    print(
        "Mapping #: %d, Precision: %.3f, Recall: %.3f, F1: %.3f"
        % (len(pred_mappings_str), P, R, F1)
    )
